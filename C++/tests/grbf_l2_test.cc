...
class GrbfL2FishPartialTest : public ::testing::Test {
protected:
    void TearDown() override {
        std::remove("output.json");
        std::remove("transformed_model.txt");
...
TEST_F(GrbfL2FishPartialTest, OutputMatchesExpected) {
    ASSERT_EQ(gmmreg_api("fish_partial.ini", "grbf_l2"), 0)
        << "gmmreg_api returned non-zero for fish_partial grbf_l2";

    std::string actual   = ReadAll("output.json");
    std::string expected = ReadAll("expected_output/fish_partial/grbf_l2.json");
    ASSERT_FALSE(actual.empty())   << "output.json was not created";
    ASSERT_FALSE(expected.empty()) << "expected_output/fish_partial/grbf_l2.json not found";
